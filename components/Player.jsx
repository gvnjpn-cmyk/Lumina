...
const hexToRgb = h => { if (!h) return '20,20,20'; const r=parseInt(h.slice(1,3),16),g=parseInt(h.slice(3,5),16),b=parseInt(h.slice(5,7),16); return `${r},${g},${b}`; };
...
export default function Player({
  ...
  const activeLine = (() => {
    ...
    for (let i = 0; i < lyrics.length; i++) {
      if (lyrics[i].time + lyricOffset <= currentTime + 0.25) idx = i; else break;
    }
  ...
  const handleShare = async () => {
    if (!cover) return;
    setSharing(true); setMenu(false);
    try {
      const url = `/api/card?cover=${encodeURIComponent(cover)}&title=${encodeURIComponent(title)}&artist=${encodeURIComponent(artist)}`;
  ...
  const downloadShare = () => {
    ...
    a.href = shareImg; a.download = `${title}-card.png`; a.click();
  };

  const BtnPlay = ({ size = 16, cls = '' }) => (
    <button onClick={onTogglePlay} className={`btn-press bg-white rounded-full flex items-center justify-center ${cls}`}
      style={{ boxShadow: `0 4px 24px rgba(${accentRgb},0.35)` }}>
      {isPlaying
  ...
  return (
    <div className="fixed inset-0 z-50 no-select" onTouchStart={onTS} onTouchEnd={onTE}
      ...
      <div className="absolute inset-0 flex flex-col overflow-hidden"
        ...
        {/* Dynamic color gradient top */}
        <div className="absolute top-0 left-0 right-0 h-72 pointer-events-none"
          style={{ background: `linear-gradient(180deg, rgba(${accentRgb},0.35) 0%, transparent 100%)` }} />

        {/* Top bar */}
        <div className="relative flex items-center justify-between px-5 pt-14 pb-6">
          <button onClick={onClose} className="btn-press w-9 h-9 rounded-full flex items-center justify-center"
          ...
          <button onClick={() => setMenu(m => !m)} className="btn-press w-9 h-9 rounded-full flex items-center justify-center"
            style={{ background: 'rgba(255,255,255,0.1)' }}>
        ...
        <div className="relative px-6 mb-6 flex items-start gap-4 flex-shrink-0">
          <div className="flex-shrink-0 artwork-shadow" style={{ width:120, height:120, borderRadius:16, overflow:'hidden', background:'#1c1c1e' }}>
            {cover
          ...
          <div className="flex-1 min-w-0 pt-2 flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              <button onClick={() => { setAddPL(true); setMenu(false); }} className="btn-press text-white/30">
      ...
      <div className="absolute inset-0 flex flex-col"
        style={{ background:'#0a0a0a', transform: lyricsOpen?'translateY(0)':'translateY(100%)', transition:'transform 0.45s cubic-bezier(0.32,0.72,0,1)' }}>
        <div className="absolute top-0 left-0 right-0 h-40 pointer-events-none"
          style={{ background:`linear-gradient(180deg, rgba(${accentRgb},0.25) 0%, transparent 100%)` }}/>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-[60]" onClick={()=>setMenu(false)}>
          <div className="absolute bottom-0 left-0 right-0 panel-in pb-8" style={{background:'#1c1c1e',borderRadius:'20px 20px 0 0',border:'1px solid rgba(255,255,255,0.08)'}}
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-6" style={{background:'rgba(0,0,0,0.85)'}} onClick={()=>setShareImg(null)}>
          <div className="flex flex-col items-center gap-4" onClick={e=>e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={downloadShare} className="btn-press px-5 py-2.5 bg-white rounded-full text-black text-sm font-semibold">Download</button>
      ...
      {addPlOpen && (
        <div className="fixed inset-0 z-[60]" onClick={()=>setAddPL(false)}>
          <div className="absolute bottom-0 left-0 right-0 panel-in pb-8" style={{background:'#1c1c1e',borderRadius:'20px 20px 0 0'}}
            onClick={e=>e.stopPropagation()}>
            ...
            <div className="overflow-y-auto" style={{maxHeight:'50vh'}}>
              {playlists.length===0
                ? <p className="text-white/30 text-sm px-6 py-4">No playlists yet. Create one in Library.</p>
                : playlists.map(pl=>(
                  <button key={pl.id} onClick={()=>{onAddToPlaylist(pl.id,track);setAddPL(false);}}
                    className="w-full flex items-center gap-4 px-6 py-3.5 text-left"
                    style={{borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
