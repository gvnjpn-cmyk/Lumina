...
export default function Player({
  ...
  const onTE = e => {
    ...
    if (dy>60) setLO(true); if (dy<-60) setLO(false);
    touchY.current = null;
  };
    ...
    <div className="fixed inset-0 z-50 no-select" onTouchStart={onTS} onTouchEnd={onTE} style={{ background:'#000' }}>
      ...
      <div className="absolute inset-0 flex flex-col overflow-hidden"
        ...
        <div className="absolute top-0 left-0 right-0 h-80 pointer-events-none"
          style={{ background:`linear-gradient(180deg, rgba(${accentRgb},0.38) 0%, transparent 100%)` }}/>
        ...
        <div className="relative flex items-center justify-between px-5 pt-14 pb-6">
          ...
          <div className="text-center">
            ...
            {sleepTimer && (
              <p className="text-white/40 text-xs mt-0.5">😴 {fmt(sleepRemaining)} left</p>
            )}
          </div>
        ...
        <div className="relative px-6 mb-6 flex items-start gap-4 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0 pt-2 flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              {spotifyUrl && (
                <a href={spotifyUrl} target="_blank" rel="noreferrer" className="btn-press text-white/30" onClick={e=>e.stopPropagation()}>
        ...
        <div className="px-5 py-4 flex-shrink-0" style={{ borderTop:'1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              <button onClick={()=>onSkip(-10)} className="btn-press text-white/50"><svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6 8.5 6V6z"/></svg></button>
              <BtnPlay size={20} cls="w-12 h-12"/>
              <button onClick={()=>onSkip(10)} className="btn-press text-white/50"><svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg></button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[60]" onClick={()=>setMenu(false)}>
          <div className="absolute bottom-0 left-0 right-0 panel-in pb-8" style={{ background:'#1c1c1e', borderRadius:'20px 20px 0 0', border:'1px solid rgba(255,255,255,0.08)' }} onClick={e=>e.stopPropagation()}>
            ...
            {[
              ...
              { label: sleepTimer?`Sleep timer: ${fmtSleep(sleepTimer)} (tap to cancel)`:'Sleep timer', icon:'😴', action:()=>{setSleepMenu(true);setMenu(false);} },
        ...
        <div className="fixed inset-0 z-[60]" onClick={()=>setSleepMenu(false)}>
          <div className="absolute bottom-0 left-0 right-0 panel-in pb-8" style={{ background:'#1c1c1e', borderRadius:'20px 20px 0 0' }} onClick={e=>e.stopPropagation()}>
            ...
            <div className="grid grid-cols-3 gap-2 px-6 pb-2">
              {SLEEP_OPTIONS.map(m=>(
                <button key={m} onClick={()=>{onSetSleep(m);setSleepMenu(false);}}
                  className={`btn-press py-3 rounded-2xl text-sm font-semibold transition-all ${sleepTimer===m?'bg-white text-black':'surface-3 text-white/80'}`}>
                  {fmtSleep(m)}
                </button>
              ))}
            </div>
            {sleepTimer && (
              <button onClick={()=>{onClearSleep();setSleepMenu(false);}} className="btn-press w-full px-6 py-4 text-white/40 text-sm text-center">
                Cancel timer ({fmtSleep(sleepTimer)} remaining: {fmt(sleepRemaining)})
              </button>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-6" style={{ background:'rgba(0,0,0,0.85)' }} onClick={()=>setShareImg(null)}>
          <div className="flex flex-col items-center gap-4" onClick={e=>e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={()=>{const a=document.createElement('a');a.href=shareImg;a.download=`${title}-card.png`;a.click();}} className="btn-press px-5 py-2.5 bg-white rounded-full text-black text-sm font-semibold">Download</button>
        ...
        <div className="fixed inset-0 z-[60]" onClick={()=>setAddPL(false)}>
          <div className="absolute bottom-0 left-0 right-0 panel-in pb-8" style={{ background:'#1c1c1e', borderRadius:'20px 20px 0 0' }} onClick={e=>e.stopPropagation()}>
            ...
            <div className="overflow-y-auto" style={{ maxHeight:'50vh' }}>
              {playlists.length===0
                ? <p className="text-white/30 text-sm px-6 py-4">No playlists. Create one in Library.</p>
                : playlists.map(pl=>(
